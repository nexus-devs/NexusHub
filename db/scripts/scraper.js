...
class Scraper {
        ...
        if(item.tags.includes('prime') && item.tags.includes('set')) {
          itemSet.forEach(item => {
            ...
            if(part[part.length-1].toLowerCase() !== 'set') {
              let relics = []
              for(var a = 0; a < item.en.drop.length; a++){
                var rarity = item.en.drop[a].name.split(" ")
                var relic = item.en.drop[a].name.slice(0, item.en.drop[a].name.length - rarity[rarity.length-1].length-1)
                relics.push({name: relic, rarity : rarity[rarity.length-1]})
            }
            components.push({name: part[part.length-1], ducats: item.ducats, relics: relics})
            }
          })
        }
      })
    ...
    isAdded(itemname, dataset) {
      ...
      let filter_type = ['vandal', 'wraith', 'prime', 'sheev', 'prime', 'xiphos', 'mantis', 'scimitar']
      let filter = ['barrel', 'blueprint', 'handle', 'neuroptics', 'system', 'chassis', 'blade', 'grip', 'link', 'systems', 'heatsink', 'wraith', 'vandal', 'blueprint', 'prime', 'hilt', 'set', 'fuselage', 'engines', 'avionics']
