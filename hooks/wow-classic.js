...
class Hook {
  ...
  async verifyContentPhases () {
    ...
    const data = [
      { contentPhase: 1, releaseDate: new Date(Date.UTC(2022, 10, 6)), description: 'Naxxramas, The Eye of Eternity, The Obsidian Sanctum' },
      { contentPhase: 2, releaseDate: null, description: 'Ulduar' },
      { contentPhase: 3, releaseDate: null, description: 'Trial of the Crusader, Onyxia\'s Lair' },
      { contentPhase: 4, releaseDate: null, description: 'Icecrown Citadel' },
      { contentPhase: 5, releaseDate: null, description: 'The Ruby Sanctum' }
    ]
  ...
  async _verifyCollection (db, collection, list, uniqueKey) {
    ...
    let bulk = db.collection(collection).initializeUnorderedBulkOp()
    for (const listItem of list) {
      ...
      if (bulk.length >= 1000) {
        await bulk.execute()
        bulk = db.collection(collection).initializeUnorderedBulkOp()
      }
    }
