...
class Crafting extends Endpoint {
   ...
   */
  async main (req, res) {
    const slug = req.params.server ? req.params.server.toLowerCase() : undefined
      ...
      return (cby) => {
        const reagents = cby.reagents.map((r) => {
          ...
          return {
            ...
            marketValue: slug ? (storage.marketValue || null) : undefined,
    ...
    const applyReagentFor = (r) => {
      ...
      return r.createdBy.map(applyData({
        ...
        marketValue: slug ? r.marketValue : undefined,
